...
class dict(object):
     ...
     def __init__(self, kernel_function, adopt_thresh, state, target, maxsize, 
                          adaptive_cleanup = True, forget_rate = .0):
          ...
          # thershold to decide how new is the incoming sample. 0 means everyone is brand new
          ...
          # a kernel function   <! yet again, this might become a higher-level class>
          ...
          # index, input and output of the recently eliminated sample
          self.eliminatedIndex = 1
          self.eliminatedDict = state
          self.eliminatedTarg = target
              ...
              if (self.numel > self.maxsize): # clean up dictionary if limit reached
                 ...
                 if (self.adaptive_cleanup):
                     ...
                     if ( float(np.random.rand(1)[0]) < self.forget_rate ):
                        ...
                        # and the span rule of Chapelle et al. (2000)
                        # Can be eighter (alpha/K_ii) or alpha/(K_ii*K_ii) or (1/K_ii) or (1/K_ii*K_ii)
                        weights = abs(Alpha.T[0] / ( np.diag(self.Kinv[:-1][:,:-1]) * np.diag(self.Kinv[:-1][:,:-1]) ))
                        ind = weights.argmin() + 1
                        
                        self.eliminatedDict = np.array(self.Dict[ind-1,:][:])
                        self.eliminatedTarg = np.array(self.Targ[ind-1])
                     ...
                     if (ind == 1):
                       ...
                       # this matrix is used to swap rows/columns
                       ...
                       # this is what we fighted for, we swap the ind and the 1st row/columns
